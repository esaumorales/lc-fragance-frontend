...
export default function AdminHomePage() {
  ...
  return <div className="flex flex-col gap-8">
    ...
    <section className="surface overflow-hidden"><div className="flex items-center justify-between border-b border-hairline p-5"><h2 className="font-serif text-2xl">Últimas actualizaciones</h2><Link href="/admin/productos" className="text-xs text-primary">Ver inventario ↗</Link></div><div className="overflow-x-auto"><table className="text-left text-sm"><thead className="text-muted-foreground"><tr><th>Producto</th><th>Precio</th><th>Stock</th></tr></thead><tbody>{data?.recentProducts.map(product => <tr key={product.id} className="border-t border-hairline"><td><p>{product.name}</p><p className="mt-1 text-xs text-muted-foreground">{product.sku}</p></td><td>{precio(product.price)}</td><td><span className={`rounded-full px-3 py-1 text-xs ${product.stock <= 5 ? "bg-primary/10 text-primary" : "bg-secondary text-foreground"}`}>{product.stock === 0 ? "Agotado" : `${product.stock} unidades`}</span></td></tr>)}</tbody></table></div>{!data && !error && <p role="status" className="p-5 text-sm text-muted-foreground">Cargando actividad…</p>}{data?.recentProducts.length === 0 && <p className="p-5 text-sm text-muted-foreground">Tu catálogo está listo para su primer producto.</p>}</section>
    {puedeEntrarAlPanel(user?.role) && <Link href="/admin/usuarios" className="surface flex items-center justify-between p-6"><div><h2 className="font-serif text-2xl">Usuarios</h2><p className="mt-2 text-sm text-muted-foreground">Quién tiene cuenta, con su dirección para coordinar envíos.</p></div><span className="text-primary">↗</span></Link>}
